...
// Get destination ID from URL and validate
$destination_id = isset($_GET['dest_id']) ? (int)$_GET['dest_id'] : 0;
if (!$destination_id) {
    // Redirect to destinations page or show an error if ID is missing or invalid
...
try {
    // Fetch destination details including banner_image
    ...
    if (!$destination) {
        // Destination not found, redirect or show error
        header('Location: our-destinations.php'); // Or maybe a 404 page
        exit;
    }
    ...
    error_log("Database error fetching resorts for destination ID $destination_id: " . $e->getMessage());
    // Maybe display a user-friendly error message
}

// Prepare the destination banner image path
...
<!-- Destination Banner with the destination's banner image -->
    ...
    <div class="banner-content">
        ...
        <p class="banner-subtitle">
            <?php 
                ...
                echo $resort_count > 0 
                    ? ($resort_count == 1 ? '1 Resort Available' : $resort_count . ' Resorts Available') 
            ...
            <div class="row">
                <?php foreach ($resorts as $resort): ?>
                    <?php
                        ...
                        $resort_folder = $resort_asset_base_path . $resort_slug;
                        
                        $resort_banner = isset($resort['banner_image']) ? $resort['banner_image'] : 'default-resort.jpg';
                        $image_path = $resort_folder . '/' . $resort_banner;

                        // Basic check if image exists
                        if (!file_exists($image_path)) {
                            $image_path = $resort_asset_base_path . 'default/default-resort.jpg';
                        }

                        // Construct the link to the resort page
