...
// Fetch existing resort details if editing (via GET parameter)
$resort = null;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Expecting an array: amenities[0]['name'] and amenities[0]['icon']
    $amenities = [];
    if (isset($_FILES['amenities'])) {
        foreach ($_FILES['amenities']['tmp_name'] as $index => $tmpData) {
            // Check if a new file is uploaded for this amenity's icon
            if (isset($_FILES['amenities']['error'][$index]['icon']) && $_FILES['amenities']['error'][$index]['icon'] == UPLOAD_ERR_OK) {
                ...
                // If no new file uploaded, keep existing icon (if passed via a hidden field)
    ...
    // Expecting an array: rooms[0]['name'] and rooms[0]['image']
    $rooms = [];
    ...
    // Process testimonials (from POST)
    ...
    // Insert or update the record in the resorts table
