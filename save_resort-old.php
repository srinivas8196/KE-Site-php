...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Create a slug for the resort (e.g., "abc")
    $resort_slug = preg_replace('/[^a-zA-Z0-9]/', '-', strtolower($resort_name));
    $resortFolderPath = "assets/$resort_slug";
    ...
    if(isset($_FILES['amenities'])){
        foreach($_FILES['amenities']['tmp_name'] as $index => $amenityTmpArray) {
            $file = $_FILES['amenities']['name'][$index]['icon'];
            $newFileName = "amenities-" . $file;
            move_uploaded_file($amenityTmpArray['icon'], "$resortFolderPath/amenities/$newFileName");
            $amenities[] = [
    ...
    if(isset($_FILES['rooms'])){
        foreach($_FILES['rooms']['tmp_name'] as $index => $roomTmpArray) {
            $file = $_FILES['rooms']['name'][$index]['image'];
            $newFileName = "rooms-" . $file;
            move_uploaded_file($roomTmpArray['image'], "$resortFolderPath/rooms/$newFileName");
            $rooms[] = [
    ...
    // Testimonials from POST (each with name, from, content)
    ...
    // Ensure $gallery and $banner_image are defined
    $gallery = isset($gallery) ? $gallery : '';
    $banner_image = isset($banner_image) ? $banner_image : '';

    if(isset($_POST['resort_id']) && !empty($_POST['resort_id'])) {
        $stmt = $pdo->prepare("UPDATE resorts SET destination_id = ?, resort_name = ?, resort_description = ?, banner_title = ?, amenities = ?, room_details = ?, gallery = ?, testimonials = ?, resort_slug = ?, banner_image = ? WHERE id = ?");
        $stmt->execute([$destination_id, $resort_name, $resort_description, $banner_title, $amenities_json, $rooms_json, $gallery_json, $testimonials_json, $resort_slug, $banner_image, $_POST['resort_id']]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO resorts (resort_name, resort_slug, resort_description, amenities, room_details, gallery, testimonials, destination_id, banner_image, banner_title) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$resort_name, $resort_slug, $resort_description, $amenities_json, $rooms_json, $gallery_json, $testimonials_json, $destination_id, $banner_image, $banner_title]);
    }
    ...
    $pageContent .= "\$resortFolder = 'assets/' . preg_replace('/[^a-zA-Z0-9]/', '-', strtolower(\$resort['resort_name']));\n";
    $pageContent .= "?>\n";
    ...
    $pageContent .= "    <h1 class=\"display-4\"><?php echo isset(\$resort['banner_title']) ? \$resort['banner_title'] : ''; ?></h1>\n";
    $pageContent .= "  </div>\n";
    $pageContent .= "</div>\n";

    // Main Content with two columns: Left for resort details; Right for destination-form.php (sticky)
    ...
    // Right Column: Destination Form (sticky sidebar)
