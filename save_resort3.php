...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Process is_active:
    // For new resorts, default to active (1) regardless of checkbox input.
    // For editing, set is_active based on whether the checkbox is checked.
    ...
    // Handle file upload
    if (isset($_FILES['resort_file']) && $_FILES['resort_file']['error'] == UPLOAD_ERR_OK) {
        $uploadDir = 'uploads/';
        $uploadFile = $uploadDir . basename($_FILES['resort_file']['name']);
        if (move_uploaded_file($_FILES['resort_file']['tmp_name'], $uploadFile)) {
            // Save resort details along with file path
            if (isset($_POST['resort_id']) && !empty($_POST['resort_id'])) {
                // Update existing resort record with file path
                $stmt = $pdo->prepare("UPDATE resorts SET file_path = ? WHERE id = ?");
                $stmt->execute([$uploadFile, $_POST['resort_id']]);
            } else {
                // Insert new resort record with file path
                $stmt = $pdo->prepare("INSERT INTO resorts (file_path) VALUES (?)");
        ...
        // Save resort details without file path
        if (isset($_POST['resort_id']) && !empty($_POST['resort_id'])) {
            // Update existing resort record without file path
            $stmt = $pdo->prepare("UPDATE resorts SET file_path = ? WHERE id = ?");
            $stmt->execute([$_POST['file_path'], $_POST['resort_id']]);
        } else {
            // Insert new resort record without file path
            $stmt = $pdo->prepare("INSERT INTO resorts (file_path) VALUES (?)");
            $stmt->execute([$_POST['file_path']]);
        }
    }

    if (isset($_POST['resort_id']) && !empty($_POST['resort_id'])) {
        // Update existing resort record (including is_active)
        $stmt = $pdo->prepare("UPDATE resorts SET destination_id = ?, resort_name = ?, resort_description = ?, banner_title = ?, is_active = ?, amenities = ?, room_details = ?, gallery = ?, testimonials = ?, resort_slug = ?, banner_image = ? WHERE id = ?");
        $stmt->execute([
            $destination_id,
            $resort_name,
            $resort_description,
        ...
        // Insert new resort record (force is_active = 1 by default)
        $stmt = $pdo->prepare("INSERT INTO resorts (resort_name, resort_slug, resort_description, banner_title, is_active, amenities, room_details, gallery, testimonials, destination_id, banner_image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $resort_name,
            $resort_slug,
            $resort_description,
    ...
    // Generate resort landing page file (e.g., yourresortslug.php)
    ...
    $pageContent .= "\$resortFolder = 'assets/' . (\$resort['resort_slug'] ?? '');\n";
    $pageContent .= "?>\n";
